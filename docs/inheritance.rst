...
child sections. Section inheritance is by default off. To turn it on,
...
With inheritance turned on in our example, section ``email.errors`` and
``email.notify`` will both have the ``server`` configuration value added
to them:
