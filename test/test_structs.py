import pytest
from conficus.structs import DoubleLinkedDict
from conficus.structs import ListNode
...
def test_double_linked_dict():
    ...
    assert len(dld) == 0
    assert dld.tail is None
    assert dld["name"] is None
...
def test_double_linked_dict_iter_names(dld):
    ...
    assert names == ["alpha", "first", "second", "beta", "third", "omega"]
